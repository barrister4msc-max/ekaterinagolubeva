...
describe("TAXOFFENCE Preview E2E gate", () => {
  test("synthetic RPC row remains factual-only and point-in-time", () => {
    ...
    expect(evidence.evidence_id).toBe(
      "fns_taxoffence:7701234567:2024-12-31:preview-taxoffence-e2e-1",
    );
    ...
    expect(evidence.attributes.observation_scope).toBe(
      "published_factual_record_not_current_liability",
    );
  });
  ...
  test("lookup uses explicit INN and preserves service response without model promotion", async () => {
    ...
    const transport = new SupabaseFnsTaxOffenceTransport({
      async rpc(fn, args) {
        ...
        return { data: [SYNTHETIC_ROW], error: null };
      },
    });
    ...
    expect(calls).toEqual([{
      fn: "fns_open_data_get_tax_offences",
      args: { p_inn: "7701234567", p_as_of_date: "2024-12-31" },
    }]);
