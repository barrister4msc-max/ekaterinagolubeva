-- Narrow writer RPC for the guarded official-origin observation step.
-- The caller cannot update legal_source_registry directly. This function validates
-- the exact five-source payload, then updates only the registry observation fields.
  ...
  loop
    ...
    if updated_rows <> 1 then
      raise exception 'guarded official-origin RPC updated % rows for source %',
        updated_rows, item.source_group_id;
    end if;
