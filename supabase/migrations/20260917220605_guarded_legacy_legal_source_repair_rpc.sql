-- Private, deliberately narrow writer for the two approved legacy-source repairs.
--
-- This function is not in an exposed API schema.  Its caller can provide only the
-- two pre-approved identities, an already verified Storage object reference, and
-- chunk text whose SHA-256 values are pinned below.  It cannot choose a source
-- type, verification/admission flag, registry row, embedding state, or a third
-- source.  All writes remain fail-closed.
...
create or replace function private.kati_guarded_legacy_legal_source_repair_audit()
returns jsonb
language plpgsql
stable
security definer
...
begin
  select
    count(distinct metadata->>'source_group_id'), count(*),
    count(*) filter (where metadata->>'is_source_head' = 'true'),
    bool_and(
      metadata->>'official_origin_verified' = 'false'
      and metadata->>'content_verified' = 'false'
      and metadata->>'temporal_verified' = 'false'
      and metadata->>'substantive_use_allowed' = 'false'
      and metadata->>'use_in_generation' = 'false'
    )
  into v_groups, v_chunks, v_heads, v_fail_closed
  from public.legal_knowledge_chunks
  where is_active is true
    and metadata->>'source_group_id' = any(array[
      '37c2c727-cc47-5e65-b60a-e43e344f89d4',
      '5428965e-92cd-52a9-a12b-0680e85adf95'
    ]);

  select count(*) into v_active_legacy
  from public.legal_knowledge_chunks
  where is_active is true
    and metadata->>'source_group_id' = any(array[
  ...
  return jsonb_build_object(
    'source_groups', v_groups, 'chunks', v_chunks, 'heads', v_heads,
    'legacy_active_rows', v_active_legacy,
    'fail_closed', coalesce(v_fail_closed, false)
  );
end;
$function$;

revoke all on function private.kati_apply_guarded_legacy_legal_source_repair(jsonb) from public, anon, authenticated, service_role;
revoke all on function private.kati_guarded_legacy_legal_source_repair_audit() from public, anon, authenticated, service_role;

do $$
begin
  if exists (select 1 from pg_roles where rolname = 'kati_internal_kb_writer') then
    grant usage on schema private to kati_internal_kb_writer;
    grant execute on function private.kati_apply_guarded_legacy_legal_source_repair(jsonb)
      to kati_internal_kb_writer;
    grant execute on function private.kati_guarded_legacy_legal_source_repair_audit()
      to kati_internal_kb_writer;
  end if;
end
$$;

comment on function private.kati_apply_guarded_legacy_legal_source_repair(jsonb) is
  'Private, atomic, hash-pinned fail-closed repair for exactly FNS БВ-4-7/3060@ and VAS Plenum 53. It cannot write legal_law_chunks, legal_source_registry, embeddings, or admission flags.';

comment on function private.kati_guarded_legacy_legal_source_repair_audit() is
  'Private aggregate-only audit for the two guarded legacy legal-source repair groups; it never returns source text.';
