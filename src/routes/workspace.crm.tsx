...
const [showCreateLead, setShowCreateLead] = useState(false);
  const [leadsData, setLeadsData] = useState(testLeads);
const [newLeadName, setNewLeadName] = useState("");
const [newLeadPhone, setNewLeadPhone] = useState("");
const [newLeadCase, setNewLeadCase] = useState("");
const [draggedLeadName, setDraggedLeadName] = useState<string | null>(null);
...
if (selectedLead?.name === draggedLeadName) {
  ...
  if (updatedLead) setSelectedLead(updatedLead);
    ...
    setDragOverColumn(null);
           ...
           className={`min-h-[260px] min-w-[320px] rounded-2xl border border-border/60 bg-white/70 p-4 shadow-[0_4px_24px_rgba(0,0,0,0.03)] backdrop-blur transition-all ${
  ...
  draggedLeadName === lead.name ? "opacity-50 rotate-1" : ""
}`}
                    >
