...
- "Пришло требование или акт проверки?"
- "Камеральная или выездная проверка?"
- "По какому налогу вопрос?"
- "Есть срок ответа в требовании?"`;
...
СТРОГО ЗАПРЕЩЕНО: придумывать конкретные статьи кодексов, номера и даты судебных дел, номера и даты писем ФНС/Минфина, реквизиты обзоров ВС РФ или иных документов. Используй только факты из обращения клиента. Если нужна правовая норма или судебная практика — упоминай область права обобщённо («нормы НК о камеральных проверках», «практика по ст. о неосновательном обогащении») БЕЗ конкретных номеров статей и дел. Конкретные источники подбирает retrieval-слой и проверяет администратор; их подтверждённость проверяется отдельно в UI.
...
Для налоговых проверок high — если есть требование ФНС со сроком ответа, акт проверки, решение о доначислении, блокировка счета, вызов в налоговую, НДС, дробление бизнеса или налоговая выгода/реконструкция.
